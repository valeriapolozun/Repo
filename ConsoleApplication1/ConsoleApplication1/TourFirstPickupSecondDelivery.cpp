...
void TourFirstPickupSecondDelivery::calcTourChoosePickupAndDeliveryPointPairs(){
	...
	{
		startNode=getPickUpDeliveryPointPairs(unvisitedNodesForOneTour, tour.back());
		if (startNode==0){
			continue;
		}
		else
...
int TourFirstPickupSecondDelivery::getPickUpDeliveryPointPairs (vector<int> unvisitedCities, int startNode)
{
    double max = -DBL_MAX;
